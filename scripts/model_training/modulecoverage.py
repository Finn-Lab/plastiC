...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-o", "--outfilepath", required=True, type=str, help="Output file path for csv"
    )

    args = ap.parse_args()
    keggmodulecov = module_coverage(args.keggdataprep)
    keggmodulecov.to_csv(
        args.outfilepath + "kegg_modules_data.csv", index=False, header=False
    )
