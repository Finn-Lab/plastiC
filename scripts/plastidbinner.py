...
def plastid_bin_stats(binpath, plastidseqlist):
    ...
    for file in binfasta:
        ...
        if bincontigcount == 0:
            ...
            percent_plastid_count = round(
                (plastidcontigcount / bincontigcount) * 100, 2
            )
        ...
        bindict[binid] = {
            "total_contig_count": bincontigcount,
            "bin_size_nt": total_nt,
            "plastid_contig_count": plastidcontigcount,
            "plastid_size_nt": plastid_nt,
            "plastid_count_percent": percent_plastid_count,
            "plastid_nt_percent": percent_plastid_nt,
        }
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-o", "--outdir", required=True, type=str, help="Path to output directory"
    )
    ap.add_argument(
        "-d",
        "--plastidbindir",
        required=True,
        type=str,
        help="Path to directory with plastid bins",
    )
    ap.add_argument(
        "-s",
        "--binsize",
    ...
    plastidbin_dataframe.to_csv(
        args.outdir + "/plastid_bin_stats.csv", index=True, header=True
    )

    filteredplastids = list(
        plastidbin_dataframe[
            plastidbin_dataframe["plastid_nt_percent"] >= args.binsize
        ]["bin_id"]
    )
