...
def summary_table(completeness, taxonomy, bins):
    ...
    taxlineage = taxonomy[
        ["# bin", "superkingdom", "phylum", "class", "order", "family", "genus"]
    ].set_index("# bin")
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "-comp", "--completeness", required=True, type=str, help="Completeness CSV"
    )
    ap.add_argument(
        "-t", "--taxonomy", required=True, type=str, help="Taxonomic classification"
    )
    ap.add_argument("-b", "--bindir", required=True, type=str, help="Bin directory")
    ap.add_argument(
        "-o", "--summaryout", required=True, type=str, help="Summary output file"
    )
    args = ap.parse_args()
